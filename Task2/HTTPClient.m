...
- (instancetype)init {
  ...
  if (self) {
    _baseURL =
        [NSURL URLWithString:[NSString stringWithFormat:@"%@?api_key=%@",
                                                        kAPIBaseURL, kAPIkey]];
    _settings =
...
- (NSURL *)prepareURLForRequestWithContactId:(NSNumber *)contactId {
  return [NSURL
      URLWithString:[NSString stringWithFormat:@"%@%@/?api_key=%@", kAPIBaseURL,
                                               contactId, kAPIkey]];
}
...
- (void)loadContactListCompletionBlockWithSuccess:(voidBlock)success
  ...
  if (self.settings.isNative) {
    if (self.settings.isSynchronous) {
      NSLog(@"Load Native Synchronos");
      [[NativeSynchronousHTTPClient sharedManager]
          loadContactListCompletionBlockWithSuccess:success
                                            failure:failure];
    } else {
...
- (void)createContactWithFullName:(NSString *)fullName
  ...
  if (self.settings.isNative) {
    if (self.settings.isSynchronous) {
      [[NativeSynchronousHTTPClient sharedManager]
           createContactWithFullName:fullName
                               email:email
    ...
    [[AFNetworkingHTTPCLient sharedManager] createContactWithFullName:fullName
                                                                email:email
                                           completionBlockWithSuccess:success
                                                              failure:failure];
  }
...
- (void)updateContact:(Contact *)contact
  ...
  if (self.settings.isNative) {
    if (self.settings.isSynchronous) {
      [[NativeSynchronousHTTPClient sharedManager] updateContact:contact
                                      completionBlockWithSuccess:success
                                                         failure:failure];
    } else {
      [[NativeAsynchronousHTTPClient sharedManager] updateContact:contact
                                       completionBlockWithSuccess:success
                                                          failure:failure];
    }
  } else {
    [[AFNetworkingHTTPCLient sharedManager] updateContact:contact
                               completionBlockWithSuccess:success
                                                  failure:failure];
  }
}
...
- (void)deleteContact:(Contact *)contact
  ...
  if (self.settings.isNative) {
    if (self.isSynchronous) {
      [[NativeSynchronousHTTPClient sharedManager] deleteContact:contact
                                      completionBlockWithSuccess:success
                                                         failure:failure];
    } else {
      [[NativeAsynchronousHTTPClient sharedManager] deleteContact:contact
                                       completionBlockWithSuccess:success
                                                          failure:failure];
    }
  } else {
    [[AFNetworkingHTTPCLient sharedManager] deleteContact:contact
                               completionBlockWithSuccess:success
                                                  failure:failure];
  }
}
