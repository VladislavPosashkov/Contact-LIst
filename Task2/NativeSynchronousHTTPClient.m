...
- (BOOL)performRequest:(NSMutableURLRequest *)request data:(NSData **)data {
  ...
  *data = [NSURLConnection sendSynchronousRequest:request
  ...
  if ([httpResponse statusCode] >= 300 || error != nil) {
    NSLog(@"Error: %@, HTTP Status: %ld", [error localizedDescription],
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
    ...
    if (data) {
      [[ContactAPI sharedManager]
          setContactList:[JSONUtilities contactListWithJSONData:data]
                  sender:self];
    }
...
- (void)createContactWithFullName:(NSString *)fullName
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
    NSMutableURLRequest *request =
        [self requestWithURL:[self baseURL]
                  HTTPMethod:@"POST"
                        body:[JSONUtilities JSONDataWithFullName:fullName
                                                           email:email]];
    NSLog(@"Req; %@", request);
    NSData *data = nil;
    BOOL isSuccess = [self performRequest:request data:&data];
...
- (void)deleteContact:(Contact *)contact
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
    ...
    NSData *data = nil;
    NSMutableURLRequest *request =
        [self requestWithURL:url HTTPMethod:@"DELETE" body:nil];
    BOOL isSuccess = [self performRequest:request data:&data];
    if (isSuccess && success) {
      dispatch_async(dispatch_get_main_queue(), ^{
        success();
