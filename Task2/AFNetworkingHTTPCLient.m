...
- (void)loadContactListCompletionBlockWithSuccess:(voidBlock) success failure:(voidBlock) failure {
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSMutableArray *contactList = [JSONUtilities contactListWithJSONObject:[(NSDictionary *)responseObject valueForKey:@"objects"]];
...
- (void)updateContact:(Contact *)contact completionBlockWithSuccess:(voidBlock)success failure:(voidBlock)failure {
    ...
    NSMutableURLRequest *request = [self requestWithURL:[self prepareURLForRequestWithContactId:contact.contactId] HTTPMethod:@"PUT" body:[JSONUtilities JSONDataWithFullName:contact.fullName email:contact.email]];
...
- (void)deleteContact:(Contact *)contact completionBlockWithSuccess:(voidBlock)success failure:(voidBlock)failure {
    NSMutableURLRequest *request = [self requestWithURL:[self prepareURLForRequestWithContactId:contact.contactId] HTTPMethod:@"DELETE" body:nil];
